...
| **История (domain)** | `MessagePayload.type` | [message-payload](../domain/message-payload.md); не путать с egress `kind` ([#127](https://github.com/skepsik/utlas-ts/issues/127), [#128](https://github.com/skepsik/utlas-ts/issues/128)) |
...
| Pin persist | код: `type: map_pin` ([#126](https://github.com/skepsik/utlas-ts/issues/126)) | `point` \| `route` \| `area` ([#127](https://github.com/skepsik/utlas-ts/issues/127)); позже document, … |
...
| Map pin (egress) | `messages` + **`MessagePayload`** | CHAT HISTORY со следующего turn — [message-payload](../domain/message-payload.md) |
...
После `show_map_pin` точка уже в ленте (`point`). Финальный `shouldReply: false` допустим. Текстовый комментарий — опционально в том же loop или на **следующем** user turn, когда `point` уже в CHAT HISTORY.

**Auto follow-up turn** сразу после pin (второй `runTurn` без user message) — **не** v0 канон; см. § Rejected / Later.

### Search vs pin

[Compose blocks](../envelope/compose-blocks.md) — TTL-кэш **результатов tools** (`message_search`, опционально `map_pin` на этапе 2). Точка **также** в CHAT HISTORY как `point` utterance — основной долгосрочный канал для «карта была отправлена».
