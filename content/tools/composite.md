...
| **Egress port** | `ReplySender.sendMapPin` | transport-нейтральный глагол доставки |
| **Telegram** | `api.sendLocation` | только в `transport/` |
| **История** | `MessagePayload.kind: "map_pin"` | сопоставление utterance с типом, не глагол |

Модели **не** светим `sendLocation` / `sendMapPin` — только стабильные tool names в prompt.
...
| Pin persist | только text `messages` | `MessagePayload` kind `map_pin` ([#65](https://github.com/skepsik/utlas-ts/issues/65)) |
| CHAT HISTORY | плоский `body` | `formatMessage` по `kind` |
...
Каждый tool: `name`, args schema, runner в `tools/runners/`. Egress-tools (pin) вызывают `ReplySender`, не grammY напрямую.
...
```text
tools/registry/
  tools.ts       атомы: schema in/out, runner ref
  chains.ts      allowed edges: mapOutputToInput(from, to)
```
...
Когда plain **`messages.text` / `MessageRef.body` недостаточно** — optional typed `payload` на ref. **Обычный текст:** `kind` null, данные в `text`. **Non-text:** колонка **`kind`** + `payload` jsonb (без `kind` внутри). Storage **собирает** / **разбирает** domain ↔ PG.

v0 в коде — только pin; при новых видах utterance union расширяем (**discriminated union** по `kind`):
...
- **Transport** мапит `sendLocation` → `map_pin` при egress ([#65](https://github.com/skepsik/utlas-ts/issues/65)).
- **PG:** колонка **`kind`** (`NULL` = plain text) + **`payload`** jsonb (поля варианта **без** `kind`). Storage **собирает** domain `MessagePayload` на read, **разбирает** на write.
...
transport/              show_map_pin runner → ReplySender.sendMapPin ([#65](https://github.com/skepsik/utlas-ts/issues/65))
storage/compose-blocks/ ToolRunResult.block (optional)
llm/prompt/format       CHAT HISTORY по MessagePayload.kind; hydrate blocks
...
«📍 lat, lon» в `body` без `kind: map_pin` (**не делаем** как канон): модель не отличает карту от болтовни; нужен `MessagePayload`.
