...
См. также: [tools](./index.md), [geocode](./geocode.md), [message-payload](../domain/message-payload.md), [envelope](../envelope/index.md), [native-tool-calls](./native-tool-calls.md), [transport](../transport/).
...
| **История (domain)** | `MessagePayload.type` (`points`, `places`) | [message-payload](../domain/message-payload.md); не путать с egress item |
| **Egress item** | `ConversationOutboundItem.kind` (`text`, `map_pin`) | transport-форма исходящего; не domain `type` |
...
**Долгосрочно** pin живёт в **CHAT HISTORY** как `MessagePayload` **`points`** — этого достаточно для «что отправили в чат».
...
| Pin persist | `points` \| `places` | document, … |
...
geocode_place ──► show_map_pin
  GeocodePlace.at → { lat, lon, label } на egress item
```
...
| Map group (egress) | `messages` + **`MessagePayload`** (`points` / `places`) | CHAT HISTORY — [message-payload](../domain/message-payload.md) |

Канон типов, PG и CHAT HISTORY — [domain/message-payload](../domain/message-payload.md). Ниже — только **когда** tool/egress пишет в слой истории.
...
После egress группы координаты/места уже в ленте (`points` / `places`). Финальный `shouldReply: false` допустим. Текстовый комментарий — опционально в том же loop или на **следующем** user turn, когда payload уже в CHAT HISTORY.

**Auto follow-up turn** сразу после pin (второй `runTurn` без user message) — **не** v0 канон; см. § Rejected / Later.

### Search vs pin

[Compose blocks](../envelope/compose-blocks.md) — TTL-кэш **результатов tools** (`message_search`, …). **`points` / `places`** в CHAT HISTORY — основной долгосрочный канал «что отправили на карту».
...
llm/prompt/format       CHAT HISTORY по MessagePayload — [domain/message-payload](../domain/message-payload.md)
...
«📍 lat, lon» в `body` без typed `MessagePayload` (**не делаем** как канон): см. [message-payload](../domain/message-payload.md) § Зачем.
...
- Denormalized `text` для typed payload (`points`) в PG (export / старые readers).
