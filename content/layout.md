...
| Тема | Решение |
|------|---------|
| Репозиторий | **npm workspaces monorepo** в `utlas-ts`: **`packages/core`** (`@utlas/core`) + **`apps/runtime`** (`@utlas/runtime`); позже `apps/tma`, …; sibling к [`ai-bot`](https://github.com/skepsik/ai-bot) |
