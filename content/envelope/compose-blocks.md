...
`blockTtl` **не** уходит в `toolCalls` (не synthetic tool). Sibling рядом с `scratchpad`, `conversationSettings`.
...
## Отличие от scratchpad

[scratchpad](./scratchpad.md) — сухой остаток совместной работы (full replace в answer). Compose blocks — кэш `messageId` + hydrate для prompt.
