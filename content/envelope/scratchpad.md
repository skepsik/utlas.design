...
- **Журнал снимков** — [declarative snapshots](./declarative-snapshots.md) (`kind: scratchpad`); effective из watermark `/forget`.
...
## Wire: поле `scratchpad`

Top-level optional в [answer envelope](./index.md) (declare-фаза; **не** `toolCalls`).
...
| Явный сброс по просьбе юзера | present + все `[]` (не omit) — [scratchpad_reconcile](#scratchpad-reconcile) |
...
```json
{
  ...
  "unresolvedQuestions": ["дедлайн не согласован"],
  "userPreferences": ["отвечать короче"]
}
```

Значение поля `scratchpad` в `LlmAnswer` — [index](./index.md#answer-envelope-canonical). Egress пользователю — только `text` (+ debug).
...
| `unresolvedQuestions` | Мета-описание незакрытых долгов, блокирующих работу | «Дедлайн не согласован», не цитата юзера |
| `userPreferences` | Мягкие пожелания по стилю или выбору | «Короче», «на ты», «сначала план» |
...
При full replace модель **пересобирает** все четыре массива; закрытый в окне долг **удаляет** из `unresolvedQuestions`.
...
- при `size > SOFT_THRESHOLD` (напр. 80% `MAX_SCRATCHPAD_SIZE`) — warning в `scratchpad_slots`: сжать, убрать устаревшее, не добавлять без удаления старого (**без** порядка слотов при truncate — см. [Rejected § truncate](#приоритет-truncate-в-промпте)).
...
- если всё ещё `> MAX_SCRATCHPAD_SIZE` → truncate по приоритету слотов (с конца низкоприоритетных):
  ...
  4. `userPreferences` — резать первыми.
...
Журнал: [declarative snapshots](./declarative-snapshots.md) — `kind = 'scratchpad'`, `payload` = validated scratchpad.

**Insert** — только после успешного turn, когда в ответе был валидный `scratchpad` (после validate / truncate).

**Load effective** — `loadDeclarativeSnapshot('scratchpad')` → compose `[[CURRENT_SCRATCHPAD_STATE]]`.

---

## Промпт

Промпты формируются **исключительно под отправляемый контекст** — PG blocks, compose включает по условиям. Имена ключей (`scratchpad_init`, …) — [turn-prompt](../turn-prompt.md) § Ключи `prompt_blocks`.
...
PG block: семантика слотов (таблица [выше](#что-хранить-и-что-нет)), «не энциклопедия», критерии [`unresolvedQuestions`](#unresolvedQuestions), cap 3–5, truncate warning при превышении порога. **Не включать** приоритет слотов при backend truncate. Точная формулировка — при authoring PG block; содержание — по этой странице.
...
### `scratchpad_reconcile` (канон) {#scratchpad-reconcile}
...
| Шаг | Действие |
|-----|----------|
| compose | load effective (`kind: scratchpad`) → `[[CURRENT_SCRATCHPAD_STATE]]` + budget/warning |
| parse | answer schema (+ optional `scratchpad`) |
| post-parse | validate → truncate |
| save | insert в [declarative_snapshots](./declarative-snapshots.md) при valid / truncated |
...
Отдельное поле «о чём говорим» или running summary (**не делаем**): якорь — `USER_MESSAGE` и `SEMANTIC_THREAD`; summary с прошлого хода даёт дубль или рассинхрон с новым сообщением.

### `facts` и энциклопедия в scratchpad

Слот `facts` и хранение общих фактов мира (**не делаем**): scratchpad — сухой остаток совместной работы (error budget), не база знаний; энциклопедическое — веса модели, tools, search.

### free-form внутренняя схема

Свободный object без жёстких полей (`unknown`, модель придумывает ключи) (**не делаем**): strict schema — четыре массива строк; иначе дрейф формата и соблазн писать summary/факты мимо слотов.

### апдейт по delta окна

Обновлять scratchpad потому что в чате новое сообщение (**не делаем**): критерий — delta **состояния** (add / remove / rephrase / явный сброс), не delta окна. Пример: decision «используем PostgreSQL» + «PostgreSQL отлично себя показывает» → omit.

### backend-фильтр `unresolvedQuestions`

Жёсткие эвристики на бэкенде (вопросительный знак, regex, «это уточнение или долг») (**не делаем**): модель видит окно целиком; отсев — промпт + cap 3–5 пунктов.

### приоритет truncate в промпте

Раскрывать модели порядок слотов при hard truncate (**не делаем**): зная, что `userPreferences` режутся первыми, а `constraints` — последними, модель может класть важное в «защищённый» слот вместо семантически верного. Слоты — типы информации, не уровни приоритета хранения. Модели: бюджет, soft warning, «убери устаревшее / сожми». Бэкенду: приоритет слотов и hard truncate.
...
Зафиксировано: strict schema (четыре массива строк); invalid → не save; truncate по приоритету слотов (`userPreferences` → … → `constraints`); truncated → save + debug. **Приоритет слотов — только backend**, в промпт не передаём (см. [Rejected § truncate](#приоритет-truncate-в-промпте)). **Числа и алгоритм — при work-issue.**
...
| 8 | Cap `unresolvedQuestions` (3–5) | Только промпт или enforce на validate (лишние → truncate/reject) |
...
## Thread scratchpad (концепт)

### Концепция

Текущий scratchpad — per-chat: модель удерживает сухой остаток чата целиком.
Следующий шаг — два слоя с разной семантикой:

| Слой | Привязка | Семантика |
|------|----------|-----------|
| `chatScratchpad` | per-chat (текущий) | Кросс-тредовое: стиль, ограничения, долгосрочные договорённости |
| `threadScratchpad` | per-SemanticThread | Сухой остаток конкретного обсуждения: решения, позиции, открытые вопросы треда |

Модель видит оба одновременно. Read/write path — отдельные поля в answer envelope.
Активация `threadScratchpad` — бэкенд, вместе с SemanticThread по триггеру.

**Проверка для модели:** *«если этот тред забыть — пункт всё равно важен?»*
— да → `chatScratchpad`, нет → `threadScratchpad`.

### Зачем

При возобновлении треда через время `threadScratchpad` подтягивается вместе с ним.
Модель входит в контекст обсуждения, а не начинает с нуля.

### Блокер: идентичность треда

`threadScratchpad` без персистентной идентичности треда не работает:
некуда сохранять, не к чему привязывать при следующей активации.

Вычисляемый ключ нестабилен — тред пополняется новыми сообщениями.
Присвоить треду id означает научиться понимать, где продолжение старого треда,
а где начало нового. Это открытый вопрос №4 (Supersede, #38) в [SemanticThread](./semantic-thread-decisions.md).

**Thread scratchpad разблокируется после решения Supersede.**

### Что остаётся открытым (к моменту реализации)

| # | Вопрос |
|---|--------|
| 1 | Схема `threadScratchpad` — те же четыре слота или другие? |
| 2 | Жизненный цикл: создаётся при первой записи или при первой активации треда? |
| 3 | Промоция: модель сама решает перенести решение из треда в chat scratchpad? |
| 4 | Архивация при закрытии треда |