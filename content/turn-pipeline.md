...
Когда появятся tools (`toolCalls` в structured answer), часть strategy (модели без надёжного JSON schema) отпадает для tool-turn'ов. Native API tools — [Rejected](./tools/native-tool-calls.md). Это **не переделка composer** — расширение turn + llm контуров.
