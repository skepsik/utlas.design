...
**Later** (не блокер v1): materialized semantic graph, post-stop append.
...
- v0 path `enrich → compose → generate` — добавить **plan** перед compose; monolith `runTurn` → orchestrator step (§ Later выше)
